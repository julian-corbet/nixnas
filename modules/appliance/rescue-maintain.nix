...
#     persisted marker that no-ops an unchanged rescue. Once the closure has landed on the
...
#   SELF-UPGRADING (rescue.flakeAttr != null): stays on an inline ukify+sbsign+place pipeline,
#     below. `nixboot.extraEntries.<name>.toplevel` is typed `lib.types.package` — a derivation
#     nixboot's flake must already know AT THIS FLAKE'S OWN EVAL TIME (see its own option doc:
#     "NO DEFAULT — guessing one would be worse than refusing to build"). A self-upgrading
#     rescue's toplevel is instead resolved AT RUNTIME, by `nix build` against whatever revision
#     `autoUpgrade.flake` currently points at — a value nixboot's declarative option surface
#     cannot represent at all, not merely one it defaults away from. This is a stated,
#     load-bearing boundary, not an oversight: every host actually running in production today
#     is hub-built (rescue.toplevel), per the operator's own build-on-hub doctrine
#     (self-upgrading boxes do local `nix build`, which the doctrine exists to avoid) — so this
#     branch exists for the option's own documented persona.
#
...
#
# HOW IT COEXISTS ON ONE ESP (see docs/HOT-MODE.md):
...
#      db key, and place+rotate it — all inline.
...
#
# NO WINDOW WITH AN INVALID RESCUE UKI: the atomic `install ... && mv -f` inside nixboot's own
# maintainer (identical shape to this file's own placement code, see modules/extra-entries.nix)
# guarantees the OLD UKI stays in place until the NEW one is fully built and signed — never
# removed first. This is a build-level guarantee, not a boot-tested one: this repo's checks
# (flake.nix) prove the wiring compiles, shellchecks, and preserves the espFileName/rotation/
# timer discipline (see `nixboot-extra-entry-rescue-equivalence` and
# `demo-hot-rescue-pinned-uki-maintainer`), but nothing here boots the real image. Any
# rescueTop change on a live host should be followed by one manual confirmation (e.g.
# `sbverify` against the placed `EFI/Linux/nixnas-rescue.efi`, or a boot to the rescue entry at
# a convenient maintenance window) — a build alone cannot prove a UKI boots.
#
...
# installed toplevel marker already matches) -- see the assertions below, which PROVE this
# structurally (never wantedBy a boot/switch target).
...
let
  ...
  maintain = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # "NO WINDOW WITH AN INVALID RESCUE UKI" note at the top of this file).
      ...
      ${lib.optionalString (cfg.rescue.toplevel != null) ''
        # PINNED: nixboot owns build+sign+place+rotate. $rescueTop above IS
        ...
        # off to nixboot's own maintainer (see modules/extra-entries.nix in the nixboot flake).
      ...
      ${lib.optionalString (cfg.rescue.flakeAttr != null) ''
        ...
        # See this file's header for why this branch stays on its own inline pipeline.
...
in
{
  ...
  config = lib.mkMerge [
    ...
    assertions = [
      ...
      # ── invariants nixboot's own extraEntries mechanism must not weaken ──
      {
    ...
    # Proved by the assertions above, not just asserted in prose.
