...
#
#   * allowReboot = false — the box must NEVER reboot itself. It is headless and the passphrase is
...
#     It activates on the next operator-initiated, passphrase-unlocked reboot, and if it misbehaves
...
in
{
  config = lib.mkIf (cfg.enable && cfg.autoUpgrade.enable && cfg.autoUpgrade.flake != null) {
    system.autoUpgrade = {
      ...
      allowReboot = false; # never self-reboot into a headless passphrase prompt
