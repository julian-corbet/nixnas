...
# boot/disk.nix, appliance/rescue-maintain.nix) by calling `import <file> { inherit
...
{ nixfsCatalogue }:
{
  imports = [
    ...
    (import ./appliance/rescue-maintain.nix { inherit nixfsCatalogue; }) # hot mode: the main keeps the stick's RESCUE current (closure + signed UKI)
